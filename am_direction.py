...
path = './'
try:
    df_ac=pd.read_csv(path+'Linear Acceleration.csv')
except:
...
I2M_idx=[ getnearIndx(IndxM, i) for i in Indx]
...
preYaw=[getYaw(magVals[I2M_idx[i]],gx,gy,gz) for i in range(len(Indx))]

#preYaw=[getYaw(magVals[I2M_idx[i]],Agx[I2M_idx[i]]-Ax[I2M_idx[i]],Agy[I2M_idx[i]]-Ay[I2M_idx[i]],Agz[I2M_idx[i]]-Az[I2M_idx[i]]) for i in range(len(Indx))]
preDir=[getDirection(preYaw[i],-20) for i in range(len(Indx))]


#preYaw = [getYaw(magVals[i], gx[i], gy[i], gz[i]) for i in range(len(gx))]
preYaw = siimp_window_filter(50, preYaw)
#preDir=[getDirection(preYaw[i]) for i in range(len(gx))]
#mof = np.mean(direction-preDir)
#plt.scatter(list(range(len(preYaw))),preYaw)
#plt.scatter(list(range(len(preYaw))),siimp_window_filter(50, preYaw))
print(sum(np.abs(preDir-direction))/len(direction))
